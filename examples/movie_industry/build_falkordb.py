"""FalkorDB graph building pipeline: ingestion + community detection.

Run this to populate the FalkorDB graph and build communities.

Usage:
  python build_falkordb.py                              # Full pipeline
  python build_falkordb.py --indexes                    # Drop and recreate indexes only
  python build_falkordb.py --llm-provider openrouter    # Use OpenRouter for LLM
  python build_falkordb.py --embedder-provider openai   # Use OpenAI for embeddings
"""
...
from recon_graphrag import CommunityPipeline, GraphBuilderPipeline
from recon_graphrag.graphdb.falkordb.index_manager import IndexManager

from config import EMBEDDING_DIM, get_embedder, get_falkordb_store, get_llm
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Build the movie-industry graph in FalkorDB."
    )
    parser.add_argument(
        "--indexes",
        action="store_true",
