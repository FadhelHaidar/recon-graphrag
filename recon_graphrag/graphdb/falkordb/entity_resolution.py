"""FalkorDB-specific entity resolution implementation.
...
because FalkorDB does not ship APOC.
"""
...
# ------------------------------------------------------------------
# Internal FalkorDB resolver
# ------------------------------------------------------------------

class _FalkorDBEntityResolver:
    ...
    def _merge_groups(
        ...
        for group in groups:
            ...
            if aliases:
                ...
                combined_props["aliases"] = existing_aliases + aliases
            ...
            merged_nodes += len(other_ids)

        return merged_nodes

    def _combine_properties(self, node_ids: list[str]) -> dict:
        rows = self.graph_store.execute_query(
            """
            MATCH (n)
    ...
    def _rewire_relationships(
        self, canonical_id: str, other_ids: list[str]
    ) -> None:
        ...
        for node_id, rel_types in types_by_node.items():
            for rel_type in set(rel_types):
                ...
                self.graph_store.execute_query(
                    ...
                    CREATE (canonical)-[new_r:{escaped_rel}]->(target)
                    SET new_r += rel_props
                    DELETE r
                    """,
                ...
                self.graph_store.execute_query(
                    ...
                    MATCH (canonical)
                    WHERE id(canonical) = $canonical_id
                    CREATE (source)-[new_r:{escaped_rel}]->(canonical)
                    SET new_r += rel_props
                    DELETE r
                    """,
                    {"node_id": node_id, "canonical_id": canonical_id},
